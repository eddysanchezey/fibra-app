...
class CalculoV2 extends Component{
    state={
        data: null,
        //idioma
        lang:2,
    ...
    searchHandlerDatPrin = () => {
        ...
        if(this.state.fechaventa.inputProps.disabled===false && this.state.fechaventa.value*1>1)
        {
            const fechaventa={...this.state.fechaventa}
            fechaventa.inputProps.disabled=true 
            this.setState({fechaventa:fechaventa})
        }
        ...
        // DATOS GENERALES   
        if(request.IdFibra>=1 && request.IdAño>1 ){
            axios.post('/calculo/buscarCalculo2',request).then(resp =>{
                if(resp.data.flag){
